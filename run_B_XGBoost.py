...
def optimize_static_params_B_XGBoost(dataset_csv,features,target,init_fit_ratio=0.2,n_trial=30, time_out=6*3600,seed=42,name='EBM_'):
    def objective_B_XGBoost(trial):
        ...
        model.fit(dtrain[0],dtrain[1], eval_set=dval, verbose=0,early_stopping_rounds=10)        
        y_pred = model.predict_proba(X_val_init)
        val_result =  roc_auc_score(y_val_init, y_pred[:,1]) 
        return val_result
    study = optuna.create_study(direction="maximize", sampler=optuna.samplers.TPESampler(seed=seed))
    study.optimize(objective_B_XGBoost,timeout=time_out,n_trials=n_trial)
    best_trial = study.best_trial
    best_p = best_trial.params
    joblib.dump(best_p,name+'_best_params')

    return best_p
def evaluate_batch_B_XGBoost(params, model, dataset, n_rows, n_delay, n_windows, features, target, metric, seed=42, moment=0, delay='delay', batch_index='batch_index'):
    ...
    for i, x,y, *kwargs in stream.simulate_qa(dataset_stream, moment=0, delay='delay'):
        ...
        if y is None:
            ...
            y_pred = [m.predict_proba(pd.DataFrame([x])[features])[0][1] for m in model]
            # add the prediction to its corresponding window
            Y_pred_buffer[index_window][i] = stats.mean(y_pred)
        ...
        if y is not None:
            ...
            if windows_count[index_window] == batch_size[index_window]:
                ...
                new_model.fit(dtrain[0],dtrain[1], eval_set=dval, verbose=0,early_stopping_rounds=10)
                model.append(new_model)
                if len(model)>3:
                    del model[0]
    return Len_Batch,result_batch,result_batch_accuracy
               
def run_experiment_batch_B_XGBoost(
                              model_name,
                              dataset,
                              dataset_name,
                              n_delays,
    ...
    if os.path.exists(save_config_dir+'best_params'):
        ...
        best_params = optimize_static_params_B_XGBoost(dataset_csv=dataset,features=features,target=target, n_trial=static_optim_ntrial, time_out=6*3600,seed=42,name=save_config_dir)
    static_params = {
              "objective":'binary:logistic',
              "eval_metric":'auc',
              "random_state":42,
    ...
    for m, number_delay in enumerate(n_delays):
        ...
        Len_Batch,result_batch,result_batch_accuracy = evaluate_batch_B_XGBoost(params=best_params,
