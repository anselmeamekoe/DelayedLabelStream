...
def optimize_static_params_LR(dataset_csv,features,target,init_fit_ratio=0.2,n_trial=30, time_out=6*3600,seed=42,name='LR_'):
    ...
    study.optimize(objective_LR,timeout=time_out,n_trials=n_trial)
    best_trial = study.best_trial
    best_p = best_trial.params
    joblib.dump(best_p,name+'best_params')
    return best_p
def evaluate_batch_LR(params, model, dataset, n_rows, n_delay, n_windows, features, target, metric, seed=42, moment=0, delay='delay', batch_index='batch_index'):
...
def run_experiment_batch_LR(
    ...
    if os.path.exists(save_config_dir+'best_params'):
        ...
        best_params = optimize_static_params_LR(dataset_csv=dataset,features=features,target=target,init_fit_ratio=init_fit_ratio,n_trial=static_optim_ntrial, time_out=6*3600,seed=42,name=save_config_dir)
    model = (
        preprocessing.StandardScaler() |
    ...
    for m, number_delay in enumerate(n_delays):
        ...
        Len_Batch,result_batch,result_batch_accuracy = evaluate_batch_LR(params=best_params,
