...
def optimize_static_params_EBM(dataset_csv,features,target,init_fit_ratio=0.2,n_trial=30, time_out=6*3600,seed=42,name='EBM_'):
    def objective_EBM(trial):
    
        params = {
                  'learning_rate': trial.suggest_loguniform('learning_rate', 1e-3, 1e-1),
                  'max_bins': trial.suggest_int("max_bins",128, 1024, step = 128 ),
    ...
    study.optimize(objective_EBM,timeout=time_out,n_trials=n_trial)
    best_trial = study.best_trial
    best_p = best_trial.params
    joblib.dump(best_p,name+'_best_params')

    return best_p
def evaluate_batch_EBM(params, model, dataset, n_rows, n_delay, n_windows, features, target, metric, seed=42, moment=0, delay='delay', batch_index='batch_index'):
    ...
    for i, x,y, *kwargs in stream.simulate_qa(dataset_stream, moment=0, delay='delay'):
        ...
        if y is not None:
            ...
            if windows_count[index_window] == batch_size[index_window]:
                ...
                ###
                model = ExplainableBoostingClassifier(**params)
                model.fit(X_train_buffer, y_train_buffer)
    return Len_Batch,result_batch,result_batch_accuracy
def run_experiment_batch_EBM(
                              model_name,
                              dataset,
                              dataset_name,
                              n_delays,
    ...
    if os.path.exists(save_config_dir+'best_params'):
        ...
        best_params = optimize_static_params_EBM(dataset_csv=dataset,features=features,target=target, n_trial=static_optim_ntrial, time_out=6*3600,seed=42,name=save_config_dir)
    ...
    for m, number_delay in enumerate(n_delays):
        ...
        Len_Batch,result_batch,result_batch_accuracy = evaluate_batch_EBM(params=best_params,
