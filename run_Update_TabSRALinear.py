...
def optimize_static_params_Upadate_TabSRALinear(dataset_csv,features,target,init_fit_ratio=0.2, n_trial=30, time_out=6*3600,seed=42,name='TabSRALinear_'):
    def objective_Upadate_TabSRALinear(trial):
    ...
    study.optimize(objective_Upadate_TabSRALinear,timeout=time_out,n_trials=n_trial)
    best_trial = study.best_trial
    best_p = best_trial.params
    joblib.dump(best_p,name+'_best_params')

    return best_p
def evaluate_batch_Upadate_TabSRALinear(params, callbacks, model,scaler, dataset, n_rows, n_delay, n_windows, features, target, metric, seed=42, moment=0, delay='delay', batch_index='batch_index'):
    ...
    for i, x,y, *kwargs in stream.simulate_qa(dataset_stream, moment=0, delay='delay'):
        ...
        if y is not None:
            ...
            if windows_count[index_window] == batch_size[index_window]:
                ...
                try: 
                    _ = model.fit_loop(X_train_buffer.values.astype(np.float32),y_train_buffer.values.astype(np.float32),epochs=100)
                except KeyboardInterrupt:
                    pass 

    return Len_Batch,result_batch,result_batch_accuracy
def run_experiment_batch_Upadate_TabSRALinear(
                              model_name,
                              dataset,
                              dataset_name,
                              n_delays,
    ...
    if os.path.exists(save_config_dir+'best_params'):
        ...
        best_params = optimize_static_params_Upadate_TabSRALinear(dataset_csv=dataset,features=features,target=target,n_trial=static_optim_ntrial,init_fit_ratio=init_fit_ratio, time_out=6*3600,seed=42,name=save_config_dir)
    ...
    for m, number_delay in enumerate(n_delays):
        ...
        Len_Batch,result_batch,result_batch_accuracy = evaluate_batch_Upadate_TabSRALinear(params=best_params,
                                                                                   callbacks=callbacks,
